...
## Vercel 即時 Demo（Preview 自動更新）

此專案可直接部署到 Vercel，並在每次推送後自動更新 Demo（含 PR Preview）。

### 1) 一次性設定
1. 到 Vercel 建立/匯入這個 GitHub repo。
2. Framework Preset 選 **Other**。
3. Build & Output 可留空（由 `vercel.json` 指定 Python 入口）。
4. 在 Environment Variables 設定：
   - `MENU_DB_PATH=/tmp/menu.db`（Vercel 無法寫入 repo，建議使用 `/tmp`）
   - `AUTH_SECRET`（正式部署請設定強隨機字串，用於簽署登入 token）
   - `AUTH_USERS_FILE=/tmp/auth_users.json`（Vercel 無法寫入 repo 時建議指定可寫路徑；正式多副本部署建議改接資料庫）
   - `AUTH_LOGIN_AUDIT_FILE=/tmp/auth_users.login_audit.jsonl`（選配；未設定時會使用帳號檔旁的分流登入稽核 JSONL）

### 2) 部署行為
- `main`（或你指定的 production branch）更新時，Production URL 會自動更新。
- 每個 Pull Request 會自動產生 Preview URL（即時 Demo）。


### 2.1) Vercel Preview：Browser-local Auth 測試模式（僅供唯讀測試）

若只是想在 **Vercel Preview / Development** 環境快速測試帳號管理 UI，但不想另外準備可持久寫入的 `AUTH_USERS_FILE`，可啟用 browser-local auth 測試模式。此模式會把測試帳號保存在目前瀏覽器的 `localStorage`，不是正式帳號資料庫。

啟用條件必須同時符合：

1. 部署環境有 Vercel 自動提供的 `VERCEL=1`。
2. `VERCEL_ENV` 是 `preview` 或 `development`，不能是 `production`。
3. 未設定 `AUTH_USERS_FILE`。
4. 明確設定 `AUTH_BROWSER_LOCAL_STORE=1`。

Vercel Preview 建議環境變數範例：

```text
MENU_DB_PATH=/tmp/menu.db
AUTH_SECRET=<請填強隨機字串>
AUTH_BROWSER_LOCAL_STORE=1
# 不要設定 AUTH_USERS_FILE
```

使用方式：

1. 開啟 Vercel Preview URL 的 `/account.html`。
2. 註冊第一個帳號；第一個 browser-local 測試帳號會自動成為 `superuser`。
3. 後續測試帳號可在同一個瀏覽器內註冊，並由第一個 `superuser` 測試帳號核准。
4. 若要清空測試帳號，請清除該 Preview 網站的瀏覽器站台資料 / localStorage。

安全限制：

- 此模式 **不會在 Vercel Production 啟用**；`VERCEL_ENV=production` 時，即使設定 `AUTH_BROWSER_LOCAL_STORE=1` 也不會簽發 browser-local token。
- 此模式 **不會在非 Vercel 環境啟用**。
- 此模式 **不適合正式部署**，因為帳號資料只存在單一瀏覽器，換瀏覽器、換裝置或清除站台資料都會遺失。正式部署請設定持久化 `AUTH_USERS_FILE`，或改用資料庫 / KV / 外部身份服務。

### 3) 本專案已包含 Vercel 入口
- `api/index.py`：Serverless 入口，掛載 FastAPI app。
- `vercel.json`：將所有路由轉發到 FastAPI，保留原本 `/`、`/admin` 與 API 路徑。

### 4) 建議：把 Demo URL 放到 README
你可以在 repo 設定完成後，補上：
- Production Demo URL
-（選配）Vercel Deploy Badge
...
- 帳號管理：註冊、登入、superuser 審核帳號；管理端寫入操作需使用 superuser 帳號登入
...
- `AUTH_SECRET`：登入 token 簽署用 secret；正式部署請設定強隨機字串
- `AUTH_USERS_FILE`：帳號資料 JSON 路徑（預設為 repo 根目錄 `.auth_users.json`，已加入 `.gitignore`）
- `AUTH_LOGIN_AUDIT_FILE`：登入/註冊稽核 JSONL 路徑（預設為帳號檔旁的 `<AUTH_USERS_FILE>.login_audit.jsonl`，避免帳號檔因 audit 寫入頻率膨脹）
- `AUTH_TOKEN_TTL_SECONDS`：登入 token 有效秒數（預設 12 小時）
- `AUTH_BOOTSTRAP_SUPERUSERS_FILE` / `AUTH_BOOTSTRAP_SUPERUSERS`：可選的初始 superuser 設定來源

Windows（PowerShell）：
```powershell
$env:MENU_DB_PATH="data\menu.db"
$env:AUTH_SECRET="change-this-to-a-strong-random-secret"
$env:AUTH_USERS_FILE=".auth_users.json"
```

macOS/Linux（bash）：
```bash
export MENU_DB_PATH="data/menu.db"
export AUTH_SECRET="change-this-to-a-strong-random-secret"
export AUTH_USERS_FILE=".auth_users.json"
```
...
### C. 管理資料（admin.html）
1. 第一次使用時，在管理頁註冊第一個帳號；第一個帳號會自動成為 active superuser
2. 後續新帳號註冊後預設為 pending，需由 superuser 在「帳號審核」中核准
3. 使用 superuser 帳號登入後再執行食材、菜色、價格、庫存、備份等寫入操作
4. 食材管理：新增/編輯食材基本資料
5. 菜色管理：新增/編輯菜色（角色、主菜類型、菜系、標籤）
6. 編輯菜色食材：維護 ingredient + qty + unit
7. 價格/庫存：用 modal 維護該食材的庫存快照與歷史價格
